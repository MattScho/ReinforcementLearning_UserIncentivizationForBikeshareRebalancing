    ...
    for users in [5000, 10000, 15000]:
        ...
        for budget in budgets:
            ...
            for i, file in enumerate(step_files):
                ...
                if v in ["p", "o"]:
                    no_opt[v][users_levels.index(str(users))] += (sum(results) / len(results))/15
                else:
                    data[str(users)][int(v)-1] += (sum(results) / len(results))/15
...
for l, data in enumerate(no_opt_data):
    plt.plot(users_levels, data, linestyle="--", linewidth=5, label=no_opt_labels[l], markersize=24)

print(data_out)
for l, data in enumerate(data_out):
    plt.plot(users_levels, data, linestyle="--", marker=markers[l], linewidth=5, label=budgets[l], markersize=24)
...
plt.xticks(range(len(users_levels)), labels=["5,000 (-50%)", "10,000", "15,000 (+50%)"], fontsize=24)
