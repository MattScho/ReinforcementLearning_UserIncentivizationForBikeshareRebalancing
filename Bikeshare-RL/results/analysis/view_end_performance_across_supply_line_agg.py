    ...
    for budget in [1000, 2000, 4000]:
        ...
        for supply in supplies:
            ...
            for i, file in enumerate(step_files):
                ...
                data[v][supplies.index(supply)] += (sum(results) / len(results))
                counts[v] += 1
print(counts)
for k in data.keys():
    data[k] = [i/9 for i in data[k]]
...
# Plot data
for k in data.keys():
    plt.plot(supplies, data[k], linestyle="--", linewidth=5, label=legend_mapper[k])
    plt.scatter(supplies, data[k], s=96)
    plt.xticks(supplies, supplies, fontsize=24)
    plt.yticks([0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0], fontsize=24)
